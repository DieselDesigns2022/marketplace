...
## Phase 12.5 live-fix verification

Run `php tests/Phase125InternalMessagingTest.php` for static security/UI contracts and `php tests/Phase105EmailsNotificationsWaitlistTest.php` for notification URL regressions. Run `RUN_DISPOSABLE_DB_TESTS=1 php tests/Phase125DatabaseIntegrationTest.php` only against disposable MySQL credentials; it creates and drops an isolated database and verifies notification opening, report-alert audiences/deduplication/reopening/privacy, blocking compatibility, and existing messaging behavior. A skip without the opt-in/database is unexecuted, not a pass. Lint every changed PHP file and run `git diff --check`.
...
Run `php tests/Phase13SponsoredPromosTest.php` for deterministic Stripe test-transport and executable view behavior. Run `RUN_DISPOSABLE_DB_TESTS=1 php tests/Phase13DatabaseIntegrationTest.php` with disposable MariaDB credentials for the real legacy migration, package snapshots, ownership/eligibility, activation replay, last-served round-robin rotation, pause/expiration notifications, weekly uniqueness/completion, and click-routing behavior. The database suite must report `SKIP/UNEXECUTED` when that opt-in environment is unavailable; a skip is not a pass.

Phase 13 integration coverage also exercises out-of-order terminal payment protection, checkout-setup cleanup, historical weekly cutoff, queued paid-promo delivery revalidation, paused/cancelled/disabled target removal, and final-appearance period eligibility. Behavioral rendering checks cover seller disclosures/history and distinct weekly versus monthly digest wording.

Weekly accounting coverage verifies that queueing, failed delivery, and suppression consume nothing; the first successful recipient delivery consumes one period appearance; later recipients add sends without another appearance; message replay is idempotent; final-period queued recipients remain deliverable; and 30 paid promos are retained with zero organic fill. Seller/admin rendering distinguishes purchased appearances, recipient sends, clicks, and website impressions.

Final correction coverage includes authoritative sent-message accounting, rejection of non-sent messages, emergency final-week removal, stable website/weekly ending warnings, category targeting without membership matching, paid-token click history, and idempotent accounting reconciliation.

The Phase 13 disposable integration suite exercises the real weekly producer path for final-appearance warnings: zero recipients produces no warning, the first successful queued recipient produces one stable warning, producer replay remains deduplicated, and exact-period appearance data recovers the warning when delivery wins the producer race.

Phase 13 integration coverage verifies that seller options and purchases accept only exact-`approved` product targets; `published` targets are rejected and excluded from website serving, historical paid clicks, weekly selection, and queued-message delivery/accounting. It also retains Category Option A coverage for an approved product promoted in a different active category.

Historical click coverage distinguishes active product, paused shop, naturally ended shop, and real service-driven admin Remove states. It verifies that pause/end links still count against eligible targets, while removed/cancelled, unpaid, and currently ineligible targets return `/browse` without incrementing clicks; separate queued-weekly tests cover both `published` and `disabled` product transitions.

Phase 13 admin-operation coverage verifies successful and repeated/invalid Pause, Resume, and Remove contracts, same-value price saves, invalid packages/prices, website paused-time extension, and weekly appearance preservation after resume. Stripe dispatch coverage uses the real controller path to verify supported Checkout Session and PaymentIntent ID mapping/activation, preservation of an existing session ID, unsupported promo-event no-ops, and non-promo event separation.

Promotion metadata-order regression coverage separately exercises supported events with missing and zero campaign IDs, unsupported events with malformed metadata, and unsupported events with a valid campaign ID. Malformed `payment_kind=promo` metadata must raise the controlled incomplete-metadata error before dispatch, while structurally valid unsupported events remain mutation-free no-ops.

The executable Phase 13 suite validates raw promotion IDs without a database, rejecting missing, empty, zero, negative, decimal, suffixed, whitespace-padded, plus-prefixed, array, and object values while allowing canonical `"42"` to reach the valid unsupported-event no-op. Disposable integration also rejects positive-looking `"1abc"` without campaign mutation.

Final live/external testing verified real Stripe promotion purchase and webhook activation, seller campaign history, website sponsored rotation, impression and click tracking, tracked destinations, admin Pause/Resume/Remove behavior, paused historical links, paused-duration extension, and removed-link suppression. Tester feedback also verified and corrected seller-facing package labels, incomplete-checkout history wording, sponsored shop-card presentation, and Promotions / Ads discoverability from the Seller Dashboard. Mobile seller-history presentation was included in external feedback review. Final main-branch smoke testing also verified that admin campaign management excludes unpaid and incomplete checkout attempts while seller history retains them.
...
Live-testing feedback identified that a newly submitted buyer review could appear visually ambiguous because the buyer was redirected directly back to the editable review. The live fix now redirects a successful new-review submission to `/dashboard/reviews`, where the published review is immediately visible. Editing an existing review continues returning to that review's detail/edit page. The specific manual redirect retest was skipped with explicit approval; it is not recorded as a passed manual test.


Run `php tests/Phase132SellerReviewsTest.php` for the fast structural regression suite. It checks delivery completion before eligibility; failed delivery not unlocking eligibility; isolated notification failures; self-review prevention; one review per order item; required review indexes; whole-star and text validation; centralized recalculation; preserved reply moderation; removed replies staying removed; report lifecycle presence; mandatory moderation notes; safe unexpected-error handling; complete Admin filters; absence of destructive seller review routes; public hiding of removed replies; and historical eligibility from `served` evidence only. Live-fix checks additionally cover nullable product/custom-service linkage, the exactly-one purchase-type constraint, successful buyer final delivery ordering, final-only Custom Design activation, and type-specific payment eligibility: normal products require `paid`, while eligible Custom Designs permit `paid` or `partially_refunded`.

Run `RUN_DISPOSABLE_DB_TESTS=1 php tests/Phase132DatabaseIntegrationTest.php` only with MariaDB credentials permitted to create and drop an isolated database. The suite applies the original Phase 13.2 migration followed by `2026_09_20_phase_13_2_custom_design_reviews_live_fix.sql` and exercises normal and Custom Design eligibility, nullable-product/custom-service persistence, duplicates, self-review prevention, shared aggregates, validation, seller authorization, reports, review/reply moderation, notifications, and historical snapshots. It proves a normal partially-refunded product remains ineligible and an eligible partially-refunded Custom Design remains reviewable. Custom historical eligibility is deliberately not fabricated; final-download evidence must be created after the live fix. When disposable MariaDB is unavailable the suite explicitly prints `SKIP/UNEXECUTED`; that result is not a pass or release verification. Codex could not execute the disposable MariaDB suite because MariaDB was unavailable in that environment. During live Phase 13.2 verification on the VPS, the disposable MariaDB integration suite was subsequently executed successfully and passed.

## Phase 13.5

Run `php tests/Phase135PromoLibraryTest.php`, lint all changed PHP, and run `git diff --check`. The fast suite checks route and permission wiring, public platform/size grouping, active/archived filtering and deterministic platform → detected-size → ID ordering, protected-file containment, upload validation contracts, bulk upload/review wiring, stale draft cleanup, preservation of entered batch details after validation errors, replacement cleanup scoping, archive behavior, escaped captions/alt text, click-to-copy captions, platform-specific social actions, downloads, and responsive CSS.

Live verification completed for the Phase 13.5 workflow includes: Admin bulk upload → per-image details → save; automatically detected dimensions; active/inactive public visibility; archive removal from Admin/public listings; public image download; full caption copy; Pinterest handoff with promo image/caption/destination; Facebook caption-copy + image-download + handoff; Instagram caption-copy + image-download + handoff; footer-only public navigation link; and mobile responsive layout. External tester repetition was intentionally skipped after owner approval because the owner completed the full Admin/public/mobile regression pass.

Live testing must use a migrated database and genuine multipart requests to verify JPG/JPEG, PNG, and WEBP acceptance; malformed, mismatched, trailing-payload, over-10-MB, over-25-megapixel, and non-upload rejection; protected-directory creation/permissions; and rollback cleanup. Verify Admin `promotions.view` versus `promotions.manage` access, CSRF rejection, bulk upload, per-image category/accessibility-description/caption entry, automatic dimension detection, preservation of entered batch details after validation errors, active/inactive visibility, stale draft cleanup behavior, image replacement without touching unrelated files, and soft archive retention/exclusion. Through real HTTP responses, verify active-only public inline/download access, inactive/archived 404 behavior, Admin preview of inactive non-archived images, content type, `nosniff`, caching, encoded download filename, and path-containment failure. In browsers on desktop and mobile, verify platform + detected-size grouping, category/caption presentation, meaningful alt text and keyboard-accessible actions, click-to-copy caption behavior, attachment download/save, Pinterest sharing, Facebook caption-copy/image-download handoff, Instagram caption-copy/image-download handoff, footer-only Promo Graphics navigation, and responsive layout. These multipart, database, filesystem, HTTP authorization/header, Clipboard API, social handoff, and visual checks remain the live verification surface; completed owner testing is recorded above.

## Social provider acceptance
After local Phase 13.6 checks, test each provider with reviewed test credentials and public HTTPS media: connect/cancel/state mismatch, revoke/expire token, choose a Pinterest board, manually post an eligible Product and active Custom Design, transition a fresh pending Product to approved once, activate an inactive Custom Design once, repeat/edit both listing types, disable each provider as Admin, repair and retry failures, and verify provider post IDs against provider dashboards. Confirm an already-active Custom Design edit does not repost, successful required preview regeneration precedes activation posting, and a regeneration failure suppresses that automatic post. Verify deleted/ineligible Product and deleted/inactive Custom Design history remains visible but not retryable. Never interpret a mocked transport result as verified provider delivery.

`Phase136SocialPublishingTest.php` is a local behavioral request-builder and transition-decision test using an injected fake provider transport. It checks secret-free URLs, paginated destination/board discovery, Pinterest board requirements, required provider response IDs, Instagram media-container readiness polling before `media_publish`, independent Meta revocation decisions, Product first-publication decisions, Custom Design inactive-to-active decisions, no repost for active edits, preview-regeneration success/failure dispatch behavior, payload construction, URL de-duplication, limits, OAuth state, disabled integrations, and the migration/schema listing-identity contract without claiming external delivery. `Phase136DatabaseIntegrationTest.php` checks `APP_ENV=test` before application bootstrap or any database connection, additionally requires `RUN_DISPOSABLE_DB_TESTS=1`, cleans up controlled fixtures, and uses a fake provider transport. Its self-contained fixtures create their own active category and IP-risk term. When enabled against a migrated disposable database it executes the actual Product approval workflows and exercises Pinterest reconnect reset, Product and Custom Design ownership/eligibility, Custom Design manual and automatic posting, retry/history, deleted or inactive listing behavior, cross-seller Custom Design denial, same-numeric-ID Product/Custom Design idempotency isolation, stored-Facebook-Page Instagram discovery, exactly-one-listing identity enforcement, attempt/failure persistence, notifications, and the persisted kill switch. These database workflow scenarios remain unverified until the guarded suite actually runs; a local skip is not a pass and real provider delivery remains separate live testing.

## Phase 14
`php tests/Phase14CollabBundlesTest.php` passed in the pre-PR Codex run. It checks fee-first equal allocation, deterministic remainder cents, current fee example, archive-name safety/collision handling, ZIP publication gating, inclusive close-date behavior, and structural database idempotency/snapshot invariants.
The guarded persistence suite is `APP_ENV=test RUN_DISPOSABLE_DB_TESTS=1 php tests/Phase14DatabaseIntegrationTest.php`. Without both the explicit guard and a disposable migrated MariaDB database it reports `SKIP/UNEXECUTED`, which is not a pass.
The fast Phase 14 suite also checks the authoritative server-backed payout estimate (including uneven remainder cents and invalid input), same-fingerprint IP decisions, fail-closed mutation invalidation, deadline guards, configured upload ceilings, refund conservation, exact-order-item download access, per-collab ZIP serialization, and deterministic archive adoption. The guarded MariaDB suite remains the authority for persistence behavior and must be run on an isolated migrated database.
Phase 14 is implemented but not production-verified until the guarded MariaDB suite runs without `SKIP/UNEXECUTED`, the migration/check constraints are applied to a disposable database, filesystem ZIP scenarios pass, and the later live Stripe workflow is verified.
The guarded Phase 14 suite now exercises persisted IP review transitions, filesystem-backed eligibility and ZIP contents/retry/hash behavior, terminal ineligibility, storefront visibility, ledger allocation, refund/recovery persistence, download policy/logging, and seller/Admin reporting queries without live Stripe calls. It remains unverified until executed on the disposable MariaDB environment.
Known unrelated failures are tracked separately and are not Phase 14 pass/fail evidence. In particular, the Phase 12.4 CSV assertion reproduced identically at pre-Phase-14 commit `8c8085c`; Phase 10.5 email naming, Phase 13 promo Admin display, and Phase 12.6 custom-final download source assertions were likewise previously base-compared. Phase 14 remains complete for pre-PR implementation review, not production/live verification.
