...
When the disposable MariaDB fixture is enabled, the Phase 12 suite executes all three CLI modes, the paid-order finalizer hook, the extracted Stripe refund-recognition path, durable 24→25→24→25→24 automatic-event cycles, stale planned-position revalidation, and authoritative Founder admin refreshes.
The final correction verifies recognition-only event JSON, authoritative Founder tenth-order/earned-date repair without false history or communication, separate durable rank events during Founder refresh, and a second no-op daily run after inactivity.
