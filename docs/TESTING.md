...
Run `php tests/Phase135PromoLibraryTest.php`, lint all changed PHP, and run `git diff --check`. The fast suite checks route and permission wiring, public platform/size grouping, active/archived filtering and platform → size → configured sort order, protected-file containment, upload validation contracts, replacement cleanup scoping, archive behavior, escaped captions/alt text, accessible actions, downloads, copy/share behavior, and responsive CSS.

Live testing must use a migrated database and genuine multipart requests to verify JPG/JPEG, PNG, and WEBP acceptance; malformed, mismatched, trailing-payload, over-10-MB, over-25-megapixel, and non-upload rejection; protected-directory creation/permissions; and rollback cleanup. Verify Admin `promotions.view` versus `promotions.manage` access, CSRF rejection, creation, metadata/caption/alt-text edits, active/inactive visibility, sort changes within a size group, image replacement without touching unrelated files, and soft archive retention/exclusion. Through real HTTP responses, verify active-only public inline/download access, inactive/archived 404 behavior, Admin preview of inactive non-archived images, content type, `nosniff`, caching, encoded download filename, and path-containment failure. In browsers on desktop and mobile, verify platform + size grouping, category/caption presentation, meaningful alt text and keyboard-accessible actions, Copy Link success/fallback, attachment download/save, Web Share/fallback, and responsive layout. These multipart, database, filesystem, HTTP authorization/header, Clipboard/Web Share API, and visual checks remain live-test requirements; the fast suite does not claim them as executed.
