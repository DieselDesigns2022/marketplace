...
After local Phase 13.6 checks, test each provider with reviewed test credentials and public HTTPS media: connect/cancel/state mismatch, revoke/expire token, choose Pinterest board, manually post, transition a fresh pending product to approved once, repeat/edit it, disable each provider as admin, repair and retry failures, and verify the provider post IDs against provider dashboards. Never interpret a mocked transport result as verified provider delivery.

`Phase136SocialPublishingTest.php` is a local behavioral request-builder and transition-decision test using an injected fake provider transport; it checks secret-free URLs, paginated destination/board discovery, Pinterest board requirements, required provider response IDs, independent Meta revocation decisions, centralized first-publication decisions, payload construction, URL de-duplication, limits, OAuth state, and disabled integrations without claiming external delivery. `Phase136DatabaseIntegrationTest.php` checks `APP_ENV=test` before application bootstrap or any database connection, additionally requires `RUN_DISPOSABLE_DB_TESTS=1`, cleans up controlled fixtures, and uses a fake provider transport. Its self-contained fixtures create their own active category and IP-risk term. When enabled against a migrated disposable database it executes the actual Admin moderation method, Admin IP-risk controller/repository flow, seller single-submission controller, seller batch-submission controller, and seller edit/save controller in isolated child processes, then verifies their persisted status and automatic-attempt results. It also exercises Pinterest reconnect reset, ownership, eligibility, attempt/failure/retry persistence, automatic idempotency and notification, the persisted kill switch, and deleted-product history. These database workflow scenarios remain unverified until the guarded suite actually runs; a local skip is not a pass and real provider delivery remains separate live testing.
