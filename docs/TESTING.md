...
- Manual coverage should confirm product price is the only buyer-facing price; license prices are not displayed as paid add-ons; Personal is always included; sellers can enable/disable Commercial, POD, Wholesale, Fabric, VA, and Extended Commercial as included permissions; buyers can select multiple included licenses; cart items store normalized selected license keys; checkout validates every selected license server-side; order items snapshot all selected included licenses with `license_price` as `0.00`; the corrective migration zeroes existing product license prices after the original Phase 8.5 migration; buyer/admin displays omit license pricing; disabled license rejection and fallback behavior work; and single-product listing cards/images do not stretch or distort.
