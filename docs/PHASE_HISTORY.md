...
Introduced resumable seller-owned collections of independent normal product drafts, per-product readiness errors, independent editing, and draft add/remove. Product 1 can selectively copy descriptions, price, category, tags, AI disclosure, complete license settings and paid prices (including POD), SEO defaults, fulfillment/manual-delivery settings, and the hand-drawn disclosure. Titles, slugs, preview images, and protected files remain product-specific and are not copied.

Submit Selected Valid Products affects only selected drafts; Submit All Valid Products considers every draft. Invalid products remain drafts with their own errors and do not block valid products. Both actions reuse normal submission behavior: clean products become `approved`; newly flagged products require product-specific seller IP-rights confirmation and, once confirmed, become `pending_review` for IP review.
