...
Confirm the order is internally completed, not fully refunded, has no unresolved refund observation or manual review, and has no Stripe charge. A reconciled partially-refunded order may still pay its positive remaining seller entitlement. Confirm the seller's connected account is approved, details-submitted, and payout-enabled. An active admin can retry the hold from **Admin → Payment Logs → Platform-credit holds**; the POST action reuses the stable Stripe idempotency key and order transfer group. If Stripe already returned a transfer ID, the retry repairs that stored successful transfer locally before applying normal refund eligibility checks and does not call Stripe again. Do not manually add a buyer `source_transaction`, edit the obligation amount, or mark the ledger transferred. The UI exposes only the sanitized retry reason; the immutable admin log retains the outcome.
