...
Bulk uploads retain the same per-file validation used by single uploads: verified PHP upload provenance, actual byte length, allowed extension/MIME agreement, decoded image/container validation, 10 MB application limit, and 25-megapixel dimension limit. New batch rows remain inactive while details are being entered. Temporary draft records use a reserved internal category, are excluded from normal Admin/public listings, and stale inactive drafts older than 24 hours are removed with their protected files.
...
### Phase 12.7 payment and refund security

Current refunds require an exact authoritative split between refundable order-item merchandise and tax; unresolved or ambiguous Stripe refunds remain in manual review without changing seller entitlement. Admin allocation, allocation retry, and recovery resolve/waive mutations are POST-only, admin-authorized, and CSRF-protected. Transactions and row locks serialize order, refund-observation, item, seller-payout, and recovery mutations so concurrent requests cannot over-allocate refunds or reserve the same recovery twice. Recovery reserved for a payout cannot be waived or resolved until that reservation finishes. Payout execution uses a durable claim/lease, fixed transfer plan, and stable idempotency key. When Stripe has already returned a transfer ID, retries repair local recovery and payout state from that stored success without issuing a duplicate Stripe transfer, including before later refund eligibility is evaluated. These controls describe the implementation and do not claim unexecuted MariaDB, live Stripe, webhook-server, browser, or deployment verification.
...
## Private messaging
Conversation, start-context, and attachment access is resolved from authenticated users and authoritative product/order ownership; submitted relationship IDs are never trusted. Product and storefront starts reject messaging one's own store. Buyer order messaging requires an order item owned by that buyer with payment state `paid` or `partially_refunded`; seller order messaging requires an order item belonging to that seller with the same payment-state allowlist.

Either participant may create a block. An active block in either direction prevents both new conversation starts and replies; only the blocker may remove their own block. Existing conversation history remains viewable, and reporting remains available while blocked.

Attachments are limited to five genuine JPG/PNG/WEBP images of at most 10 MB each and are checked by upload status, extension, Fileinfo, and decoded image metadata. Random names, non-public storage, containment/readability checks, `nosniff`, and participant authorization protect files. Admin attachment access requires authenticated administrator authorization and an existing report, and the requested attachment must belong to the conversation referenced by the supplied report ID. Bodies render as escaped plain text. Messaging never grants product download access.

### Corrected attachment and retention controls
Messaging uploads must be genuine PHP multipart uploads with complete aligned arrays, successful upload status, an allowlisted matching extension/MIME, 1–10 MB size, at most 25 million source pixels, exact parsed JPEG EOI / PNG IEND+CRC / WEBP RIFF boundaries, and successful GD decode. This rejects trailing/polyglot data before randomized protected storage. Failed persistence removes newly stored files. Retained messaging history blocks permanent deletion before its transaction; block relationship rows retain their schema-defined cascade behavior.
## Custom-design files
Reference, proof, and final files are stored below `storage/protected_uploads/custom_designs`, validated by upload provenance, size, and server-detected MIME. File responses re-check canonical containment and participant ownership; final files additionally require the owning buyer's paid or partially-refunded order. A fully refunded order retains seller/admin history but revokes buyer final-download access and does not render a final-file URL.
Image uploads additionally use the marketplace's exact-container and dimension checks. Transaction failures remove newly moved files, responses use private/no-store attachment headers, and sellers cannot access pending or abandoned briefs or references. Completed custom deliveries are also listed in the existing protected buyer order/download experience.

### Phase 12.6 active delivery corrections
Seller historical visibility includes fully refunded custom orders, but workflow mutations and buyer final downloads require `paid` or `partially_refunded`. Cancelled custom requests are terminal and cannot enter generic payment retry. The custom-file controller is confined to the canonical `storage/protected_uploads/custom_designs` subtree.

Examples, reference images, and proof images require strict extension/MIME agreement plus image/container/dimension validation; reference/proof PDFs require the PDF extension/MIME pair. Final delivery has no business-format allowlist, but differs from the older normal-product uploader: at most 10 files of at most 25 MB each are accepted; arrays must be aligned; PHP status must be successful; `is_uploaded_file` provenance and actual-size agreement are mandatory; and the lowercase-normalized extension must be non-empty, alphanumeric, and 1–12 characters. Fileinfo MIME is detected and stored as metadata but is not a final-format allowlist. Original names are bounded, stored names are randomized, bytes remain protected, and participant authorization is required.

Required brief fields (`design_request`, `required_text`, `design_direction`), optional color/dimension/format/use/notes, and seller questions with their required flags and answers are retained in the private brief snapshot. Service title, description, seller instructions, agreed price, turnaround, and revisions are separately snapshotted. Only service examples are public; buyer references are protected.
...
Review creation revalidates paid order ownership and item-level successful-download eligibility server-side, and rejects self-reviews by comparing the authenticated buyer with the designer owner. Seller reply/report actions join through the authenticated seller's designer record. Reporting never automatically hides a review, and the database unique constraint prevents the same seller from reporting the same review more than once. All buyer review and seller-response content is escaped on output.

`reviews.view` is read-only. Review moderation, report resolution, and seller-reply removal/restoration require `reviews.manage` and CSRF validation. Review and reply moderation require both a reason and a nonblank internal note. Moderation preserves the original content and an audit history; report resolution preserves the acting Admin, status/outcome, notes, `reviewed_at`, and `resolved_at` where applicable. Removing a review may transition its active reports to `review_removed`.

Seller-response removal and restoration are Admin-only operations. Removed responses remain stored but are hidden from public and buyer displays, and seller editing cannot silently republish an Admin-removed response. Download eligibility is activated only after `readfile()` reports successful completion; post-delivery eligibility and notification failures are logged without exposing internal errors or invalidating delivered content. Unexpected review-workflow exceptions are sanitized.

## Social publishing
Social OAuth uses authenticated initiation, normal form CSRF verification, and a single-use random callback state with a ten-minute lifetime. Access tokens are authenticated-encrypted using Sodium and `SOCIAL_CREDENTIAL_ENCRYPTION_KEY`; secrets/tokens are excluded from views, URLs, notifications, and application error logging. Instagram discovery may decrypt an existing connected Facebook Page credential server-side to query that Page and its linked professional account directly; the Page token is sent only in `Authorization` headers and is never placed in a request URL. Every mutation scopes records to the current approved designer. Product ownership, approved/published status, and `product_images` ownership are validated immediately before publication. Custom Design ownership, active status, and `custom_service_images` ownership receive the equivalent validation. Provider calls use bounded timeouts and sanitized failure storage.

Each `social_post_logs` row is constrained by `social_post_listing_identity_chk` to identify exactly one Product or Custom Design. Listing and image IDs intentionally have no foreign keys that would erase or block retained posting history when a listing or preview is deleted; the durable title snapshot keeps that history readable, but retained IDs never restore posting or retry eligibility.

## Collab bundle security
Only authoritative approved designers can host or participate. Mutations enforce CSRF, host/membership/file ownership, and deadline locks. Invitation tokens are stored only as SHA-256 hashes. Source files and final ZIPs live below protected storage; final delivery verifies order ownership, containment, and the snapshotted SHA-256. Archive entry names use basenames, restricted characters, and database IDs to prevent traversal and collisions. IP-risk matches block publication.
Phase 14 invitation acceptance is a CSRF-protected POST after an authenticated GET confirmation. Share tokens and direct-email tokens are stored only as hashes and expire at the File Upload Deadline. IP screening covers title, description, and current contribution/Terms filenames; clean content has no routine Admin approval. Risk matches persist for Admin review. A material change first invalidates the prior decision; an unchanged fingerprint preserves its applicable decision, while a failed rescan leaves publication blocked. Admin review is fingerprint-bound and stale reviews are rejected.
Collab uploads use the deployment's authoritative PHP `upload_max_filesize` and `post_max_size` ceiling, reject executable/web-active extensions, and restrict Terms/license uploads to PDF, TXT, RTF, DOC, DOCX, or ODT. Randomized stored names never reuse the submitted path.
Protected collab ZIP access is authorized against the exact owned order item, including item-level refund allocation. Path and SHA-256 checks precede delivery, and the served download row and item counter are written only after `readfile` reports success.
The interactive payout calculator calls an approved-seller-only, CSRF-protected endpoint. Price and designer count are parsed server-side, and the response is computed by `MarketplaceFeeService::configured()` through `CollabPayoutService`; calculator requests cannot mutate collab state.
Protected collab contributions allow SVG alongside other non-executable design formats; files remain below protected storage and are served only as attachments. SVG is not accepted as a Terms/license document. PHP/PHTML/PHAR and other executable or web-active formats remain blocked. Replacement writes and commits the new owned row before removing the old protected file; deletion/replacement use containment checks. ZIP assembly rechecks every snapshotted source SHA-256, sanitizes entry basenames, and prefixes file IDs to prevent traversal and collisions.
An eligible contributor must be the host or an accepted participant, meet the contribution-file minimum, and have a Terms/license file. Terms do not count toward that minimum. Excluded participants receive no payout, contribute no customer-ZIP files or Terms, and are omitted from active storefront exposure while their protected submission/history records remain retained.
