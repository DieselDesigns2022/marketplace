...
Automatic recognition transitions use an insert-only event ID rather than a final-state hash. Stable paid/refund trigger keys are unique and bounded; replay recovers communication from the committed event. Proposed Founder positions are recomputed after seller/mutex locking before assignment.
