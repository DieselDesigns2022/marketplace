...
## Social publishing
Social OAuth uses authenticated initiation, normal form CSRF verification, and a single-use random callback state with a ten-minute lifetime. Access tokens are authenticated-encrypted using Sodium and `SOCIAL_CREDENTIAL_ENCRYPTION_KEY`; secrets/tokens are excluded from views, URLs, notifications, and application error logging. Every mutation scopes records to the current approved designer. Product status and preview-image ownership are validated immediately before publication. Provider calls use bounded timeouts and sanitized failure storage.
