...
All batch routes require an approved seller with completed onboarding. Every batch lookup is constrained by `designer_id`; item operations join batch membership, product, and designer ownership. Another seller receives a not-found response and cannot add, remove, copy, or submit against the batch. Removal is limited to order-free drafts. The router's global POST CSRF verification applies to batch creation and every mutation.

Copying is allowlisted to supported shared fields and only targets drafts owned through the same batch; it creates no live template relationship. Submit Selected and Submit All Valid evaluate each product independently for stored product, category, preview-image, protected-file/manual-delivery, license-price, and IP-risk requirements. Both use the same authoritative submission service as single-product submission: clean products become `approved`; a newly flagged product stays a draft until that product's seller IP-rights confirmation is supplied and then becomes `pending_review` for IP review. One invalid or failed product does not roll back another product's successful submission.
