...
Alerts use the durable type-aware conversation snapshot for products, storefronts, and purchased order items. In-app notification action URLs are validated local Asset Moth paths; transactional email conversation links are absolute Asset Moth URLs. Alerts never expose message bodies, attachment contents, or participant email addresses.
