...
## Scope
Phase 10.5 adds durable in-app notifications, the public launch waitlist, consent-aware promotional campaigns, and a database-backed email queue. Purchase mail is queued only after committed order finalization, whether triggered by a verified Stripe payment or an internally completed fully credit-funded order. Refund, seller sale/coupon, seller application, product moderation, Stripe Connect setup/payout-readiness transitions, failed-payment, and waitlist events have idempotent hooks. Tax-reminder, promotion, bundle, and rank/badge methods remain foundation-only in this Phase 10.5 section. Phase 11 now uses the credit/referral notification helpers described below.
...
Paid-order receipt queue data includes the order-item seller receipt snapshots. Purchase-receipt email items are grouped by seller, and only that group's allowlisted image and escaped plain-text note appear in its seller section. Order number, date, payment status, licenses, stored prices, coupon, tax, final total, and order link remain Creative Moth-controlled. Notification, consent, waitlist, campaign, deduplication, and retry behavior is unchanged.

### Phase 11 notification hooks
Phase 11 uses the Phase 10.5 queue/idempotency foundation for buyer referral reward (`referral:{id}:buyer:notify:{participant}`), seller referral reward (`referral:{id}:seller:notify:{participant}`), admin adjustment (`admin-credit:{admin}:{nonce}:notification`), paid receipt/download readiness, and one seller-sale event per seller/order. Replay-safe event and email deduplication keys prevent duplicate communications.

Phase 11 financial finalization commits before receipt, sale, coupon, download, or referral communications are queued. Each communication uses its stable order/referral key; sanitized reporting records a queue failure without rolling back payment, credit, tax, payout, fulfillment, or rewards.
...
### Phase 12 recognition communications
Automatic Silver, Gold, Platinum, and Diamond achievements use email type `creator_rank`, a rank-focused subject/title, seller name/message, and `/seller/rank` action URL. Automatic Founder earning/reactivation, admin Founder grants, eligible restores, and inactive-to-active force actions use `founder_badge` with Founder-focused copy and the same safe action URL. An upward admin displayed-rank assignment uses `creator_rank`; lowering, locking the same effective rank, and removing an override are notification-only and never use a Founder subject.

Refund downgrades, automatic Founder inactivity, rank-override removal, and forced inactivity are notification-only. Backfill calls disable communications. Automatic notification and email keys derive from durable recognition-event IDs, while admin keys derive from `admin_logs.id`; stable paid/refund trigger keys recover missing communication on replay only when current semantic state matches and the event-linked rank/badge history is still latest across automatic and administrative transitions. Replayed hooks, daily recalculation, and no-op admin submissions cannot enqueue duplicates. Queue/reporting failures remain post-commit and cannot reverse recognition or financial state.
...
After the message transaction commits, the application attempts both an in-app notification hook and a transactional, marketing-preference-independent email queue hook. Both hooks use stable deduplication identities derived from the durable message and recipient IDs. Failures are sanitized and logged, and a hook failure does not roll back the committed message.

Alerts use the durable type-aware conversation snapshot for products, storefronts, and purchased order items. In-app notification action URLs are validated local Creative Moth paths; transactional email conversation links are absolute Creative Moth URLs. Alerts never expose message bodies, attachment contents, or participant email addresses.
...
Paid custom requests notify the seller. Proof availability, revision requests, approvals, status changes, and final availability use deduplicated `NotificationService` events; conversation replies continue to use Phase 12.5 internal-message notifications and email queue behavior.
Workflow notification keys are based on durable history IDs. Notification failures after commit are reported through `NotificationService::reportFailure` and cannot roll back or misreport an already committed workflow transition.
Initial custom-order payment queues the purchase receipt but suppresses the normal-product “downloads ready” email. The final-availability notification and transactional email are queued only after the seller commits final delivery.
Final in-app notification and custom-final email are independent post-commit attempts with history-based deduplication; either failure and its reporter failure are contained without suppressing the other attempt or changing the committed workflow.
Shared paid-order finalization retains the buyer purchase receipt and the normal seller sale notification/email, and additionally creates the custom-order seller notification. Only the buyer's premature normal-product `download_ready` communication is suppressed. When final delivery commits, the application independently attempts the buyer `custom_final_available` notification and dedicated `custom_final_available` transactional email; failure of either does not suppress the other, and both identities derive from the durable custom-order history ID.
# Weekly paid promotions

Phase 13 promotion payment and activation status uses the seller's in-app `promo_status` notification. A separate transactional email confirming paid-promotion activation is not sent. Weekly Email promotion delivery remains part of the existing marketing-email system and Weekly Emails preference.

The existing Weekly Emails preference controls the marketplace digest. Phase 13 allows unlimited eligible paid promotions and renders all of them first. Randomized marketplace-wide organic products fill the remaining space toward the normal 24-card target: 3 paid promos permit up to 21 organic cards, while 24 or 30 paid promos retain all paid promos and use zero organic cards. Paid product targets are excluded from the organic fill. Monthly and favorite-shop behavior is unchanged.

Immediately before weekly delivery, queued paid promotions are rebuilt from current authoritative shop/product data and removed if the campaign is paused/cancelled, unpaid, or no longer has an approved seller and eligible target. A purchased appearance is consumed only by the first successful recipient delivery containing that promo for the weekly period—not by queueing. Every successfully delivered recipient email containing the promo increments its send total exactly once. A campaign ended by its final consumed appearance remains deliverable to other already-queued recipients only for that exact recorded weekly period. Purchases cannot be inserted retroactively into a digest period whose end date predates payment. Monthly digest wording and content selection remain unchanged.

When a successfully produced weekly issue includes a campaign with exactly one purchased appearance remaining, the seller receives the stable `promo:{id}:ending` warning. An administrator may use emergency Remove after the first final-week delivery to prevent remaining pending recipients from receiving that promo; already sent or in-transport mail is not recalled.

The final-appearance warning is attempted immediately after the first recipient for that weekly issue successfully queues. If delivery wins that race and ends the campaign first, the same stable warning is recovered only when the exact issue period already has its authoritative appearance row.

Phase 13 paid product promos are selected and revalidated using the target product’s current exact `approved` status. `published` products are excluded from new weekly issues and removed from queued weekly messages before delivery, so their removal consumes no appearance and records no recipient send. Approved products may still advertise in any active category, independent of product category membership.

Emergency admin Remove continues to cancel remaining queued final-week promo delivery and now also permanently disables that campaign’s tracked links. Already recorded sends remain intact; later removed/cancelled-link requests return `/browse` and add no clicks.
