...
Either participant may create a `message_blocks` row, but only the user who created that block may remove it. `message_reports` permits the reasons `abuse`, `spam`, `inappropriate`, and `other`; moderation status is `open`, `reviewing`, `resolved`, or `dismissed`. There is one current report per participant/thread. `message_reports.notification_cycle` is durable moderation-alert cycle state: a new report begins a cycle, repeat submissions while it remains `open` or `reviewing` retain that cycle, and participant reopening after `resolved` or `dismissed` advances it while clearing the prior moderator identity, moderator notes, and reviewed timestamp. The report ID and cycle form the deterministic admin-notification deduplication identity. Admin-notification copy excludes report details, message contents, attachments, and participant email addresses. Messaging participant/sender/reporter/moderator foreign keys are retained history: permanent account deletion detects them before beginning deletion rather than removing that history.
## Phase 12.6 custom designs
`custom_design_services`, `custom_service_questions`, and `custom_service_images` remain separate from products. `custom_orders` links one standard order/order item to immutable JSON service and brief snapshots; `custom_order_files` and `custom_order_status_history` retain protected delivery and workflow history. `message_conversations.custom_order_id` supplies order context.
System-driven history rows have a nullable actor and an explicit `transition_source`; user actions retain their authenticated actor. Seller queries expose only legitimately paid or historically paid/refunded custom orders.
Custom-design checkout uses the existing `orders` credit snapshot columns and Phase 11 reservation/finalization/release ledger. Coupons remain unapplied because the existing coupon scope is product/cart-specific.
Cancelled custom requests are not retryable. Failed payments may be retried only while the custom request remains `new`; ordinary product retry behavior is unchanged.
Payment cancellation/expiration and full-refund terminal transitions share one transaction with standard order state; system history uses a unique event identity for replay repair. Platform commissions identify custom rows by `custom_service_id` and use NULL-safe product/custom comparisons during finalization recovery.
The user workflow is seller `new -> in_progress`, seller `revision_requested -> in_progress`, seller `in_progress -> proof_review`, buyer `proof_review -> revision_requested`, buyer `proof_review -> in_progress` (proof approval), and seller `in_progress -> completed`. Each accepted buyer revision request increments `revisions_used` exactly once. Usage may exceed the snapshotted `included_revisions` if the seller continues, but no overage charge is created. `completed`, `cancelled`, and `refunded` are terminal for user actions; a legitimate system full refund may transition `completed -> refunded`.

`custom_order_status_history.system_event_key` is NULL for user-driven history and contains a durable source-plus-terminal-target identity for system transitions. The unique `(custom_order_id,system_event_key)` key lets cancellation/refund replay repair missing state/history without duplicating a system row. `platform_commissions.custom_service_id` is nullable, indexed, and foreign-keyed to `custom_design_services`; commission grouping is `(order_id,product_id,custom_service_id,designer_id)`, and recovery compares both `product_id <=> ?` and `custom_service_id <=> ?` so NULL product IDs neither duplicate nor conflate custom services.
...
Migration `database/migrations/2026_09_18_phase_13_2_seller_reviews.sql` adds `order_items.downloaded_at`, `order_items.review_eligible_at`, and `order_items.reviewed_at`. The subsequent live-fix migration `database/migrations/2026_09_20_phase_13_2_custom_design_reviews_live_fix.sql` must run after that original migration. Historical normal-product eligibility is backfilled only from reliable `downloads.status='served'` evidence; denied or otherwise non-served records do not qualify. The live fix does not fabricate historical Custom Design download evidence: an existing completed Custom Design becomes eligible only when its buyer successfully downloads a final file again.

`seller_reviews` permanently links the buyer, designer, order, and order item to exactly one purchase type. `product_id` is nullable for Custom Design reviews, while `custom_service_id` stores their restrictive custom-service linkage; `seller_reviews_purchase_link_check` requires exactly one of those columns to be non-NULL. Buyer, seller, and purchased-item snapshots remain retained. Moderation statuses are `published`, `under_review`, and `removed`. A unique constraint on `order_item_id` continues to enforce one review per purchased order item. Independent indexes cover designer, buyer, product, custom service, order, rating, moderation status, `reviewed_at`, and `created_at`, with an additional public seller/status/date index.

Custom Design eligibility is activated only after successful delivery of a completed final file to the owning buyer and an eligible `paid` or `partially_refunded` payment. Proof/reference access, seller or Admin access, failed delivery, and repeated downloads do not create additional eligibility; repeated successful final downloads are idempotent. Normal products continue to require `payment_status='paid'`.
...
`social_platform_integrations` stores independent operational kill switches. `seller_social_connections` stores one encrypted provider credential bundle and preferences per seller/platform, including connection health and Pinterest board selection. `social_post_logs` is a durable, append-per-attempt posting ledger: every manual post, automatic attempt, and retry receives its own row. Each row identifies exactly one normal product (`product_id`) or Custom Design listing (`custom_service_id`) and snapshots its title so history remains readable after listing edits or deletion. A new attempt begins as `attempting`; that same row is finalized as `succeeded` or `failed` with its completion time, provider post ID when available, or sanitized failure details. Retries create separate rows linked through `retry_of_id`, while the nullable unique type-prefixed `automatic_key` enforces one automatic attempt per listing/platform without collisions between listing types.

The posting ledger identifies either a normal Product or a Custom Design, with the `social_post_listing_identity_chk` constraint requiring exactly one of `product_id` or `custom_service_id`. `listing_title` is the durable title snapshot used to keep seller-scoped history readable after a listing changes or is deleted. Numeric listing and image references intentionally have no listing/image foreign keys that would prevent retained history after deletion. Retained history never restores posting eligibility: a Product retry requires the Product to still exist and be `approved` or `published`, while a Custom Design retry requires the Custom Design to still exist and be active. Deleted or inactive listings remain visible in history but are not retryable.

## Phase 14 collab bundles
Migration `2026_09_23_phase_14_collab_bundles.sql` and canonical `database/schema.sql` add `collab_events`, `collab_participants`, `collab_invitations`, `collab_files`, `collab_ip_risk_detections`, `collab_ip_risk_reviews`, and `collab_order_allocations`. `collab_events` stores the host, Open/Closed type, minimum, File Upload Deadline, Sale Start, Close Date, IP state/fingerprint, immutable `snapshot_at`/`eligible_count`, and final archive path/SHA-256/error/attempt/readiness state. `collab_participants` snapshots eligibility, qualifying contribution count, exclusion reason, and snapshot time; `collab_files.included_in_snapshot` freezes archive membership while retaining excluded uploads securely.
Phase 14 order items enforce exactly one Product, Custom Design, or Collab identity and optionally snapshot the discovery storefront. Downloads retain Product behavior while using a mutually exclusive `collab_id` for the shared ZIP; authorization and logs correlate to the exact `order_item_id`. Allocation uniqueness is `(order_item_id, designer_id)`, allowing the same contributor to participate in multiple collabs in one order. `collab_order_allocations` records collab/order/item, contributor, qualifying count, eligible-count snapshot, gross basis, the one shared marketplace fee, contributor pool, individual/refunded allocation, and optional storefront source.
Collab relationships on `seller_earnings` and `platform_commissions`, plus the existing per-order/designer `seller_payouts` and refund-recovery records, provide the financial audit trail. The marketplace fee is calculated once for the shared order item; it is not independently charged to each contributor. Partial, successive, and full merchandise refunds recalculate that one remaining fee and equal contributor pool, then synchronize allocation refunds, earnings, payout fee components/commission/net amount, platform commission, and transferred-payout recovery obligations without duplicating gross sale revenue.
`collab_events.status=ineligible` is the terminal deadline outcome when no host/accepted participant satisfies both the minimum contribution-file requirement and required Terms/license file. Terms files do not count toward the minimum. The terminal state is distinct from retryable `failed` ZIP infrastructure state, preventing cron retry loops.
