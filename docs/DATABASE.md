...
`social_platform_integrations` stores independent operational kill switches. `seller_social_connections` stores one encrypted provider credential bundle and preferences per seller/platform, including connection health and Pinterest board selection. `social_post_logs` is a durable, append-per-attempt posting ledger: every manual post, automatic attempt, and retry receives its own row. A new attempt begins as `attempting`; that same row is finalized as `succeeded` or `failed` with its completion time, provider post ID when available, or sanitized failure details. Retries create separate rows linked through `retry_of_id`, while the nullable unique `automatic_key` enforces one automatic attempt per product/platform.

The posting ledger intentionally retains the numeric product and image references without foreign keys that would prevent later product cleanup. Seller-scoped history uses the current product title when available and a `Deleted product #ID` fallback after deletion. A retained history row does not restore posting eligibility: retry remains available only when the current product exists and is `approved` or `published`.
