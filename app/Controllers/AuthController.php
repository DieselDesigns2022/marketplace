...
class AuthController {
 ...
 public function login(){ $error=null; if($_POST){$u=DB::row('select * from users where email=? and status="active"',[$_POST['email']]); if($u&&password_verify($_POST['password'],$u['password_hash'])){$_SESSION['user']=['id'=>$u['id'],'name'=>$u['name'],'email'=>$u['email'],'role'=>$u['role']]; H::redirect('/dashboard');} $error='Invalid credentials';} H::view('auth/login',compact('error')); }
