...
final class ReviewController
{
    ...
    public function buyerForm($id):void
    {
        ...
        $sql='select oi.*,o.created_at purchase_date,coalesce(oi.product_title,p.title,"Purchased item") product_title,coalesce(oi.seller_name,d.display_name,"Seller") seller_name,d.store_slug,(select image_path from product_images pi where pi.product_id=oi.product_id order by pi.sort_order,pi.id limit 1) preview_image from order_items oi join orders o on o.id=oi.order_id left join products p on p.id=oi.product_id left join designers d on d.id=oi.designer_id where oi.id=? and o.user_id=?';
        $item=$review?DB::row($sql,[$review['order_item_id'],$uid]):DB::row($sql.' and o.payment_status="paid" and oi.review_eligible_at is not null',[(int)$id,$uid]);if(!$item)H::abort(404);
