...
class SellerController
{
    ...
    private function refundSummary(int $designerId): array
    {
        ...
        foreach(DB::rows('select sp.order_id,sp.original_seller_payout_amount,sp.seller_payout_amount,coalesce(sum(a.merchandise_refund_cents),0) refunded_cents from seller_payouts sp left join order_items oi on oi.order_id=sp.order_id and oi.designer_id=sp.designer_id left join marketplace_refund_allocations a on a.order_item_id=oi.id where sp.designer_id=? group by sp.order_id,sp.original_seller_payout_amount,sp.seller_payout_amount',[$designerId]) as $row){$orderId=(int)$row['order_id'];$original=StripeService::cents($row['original_seller_payout_amount']??0);$current=StripeService::cents($row['seller_payout_amount']??0);$sellerRefund=max(0,$original-$current);$summary['gross_refund_cents']+=(int)$row['refunded_cents'];$summary['seller_refund_cents']+=$sellerRefund;$summary['by_order'][$orderId]=$sellerRefund;$summary['original_by_order'][$orderId]=$original;}
        return $summary;
    }
    ...
    public function saleDetail($id)
    {
        ...
        $item=DB::row('select oi.*,o.user_id buyer_id,o.status order_status,o.payment_status,o.created_at order_created,u.email buyer_email,u.name buyer_name,sp.recovery_reserved_amount,sp.recovery_applied_amount,sp.transfer_amount_after_recovery,sp.stripe_transfer_id,(select co.id from custom_orders co where co.order_item_id=oi.id) custom_order_id from order_items oi join orders o on o.id=oi.order_id join users u on u.id=o.user_id left join seller_payouts sp on sp.order_id=oi.order_id and sp.designer_id=oi.designer_id where oi.id=? and oi.designer_id=? and o.payment_status in ("paid","partially_refunded")',[(int)$id,$d['id']]) ?? H::abort(404);
