...
class SellerController
{
    ...
    {
        return DB::row(
            'select * from designer_applications where user_id=? order by created_at desc, id desc limit 1',
            [H::user()['id']]
        );
    }
    ...
    {
        return (bool) DB::row(
            'select id from designers where store_slug=? and id<>? limit 1',
            [$slug, $designerId]
        );
    }
    ...
    private function applicationValues(): array
    {
        return [
            ...
            'desired_slug' => trim($_POST['desired_slug'] ?? ''),
            'bio' => trim($_POST['bio'] ?? ''),
            'portfolio_url' => trim($_POST['portfolio_url'] ?? ''),
            'social_links' => trim($_POST['social_links'] ?? ''),
            'design_types' => trim($_POST['design_types'] ?? ''),
            'uses_ai' => trim($_POST['uses_ai'] ?? ''),
    ...
    public function apply()
    {
        ...
        if ($_POST) {
            ...
            $errors = $this->validateApplication(
                $values,
                $application && $application['status'] === 'denied' ? (int) $application['id'] : null
            );

            if (!$errors) {
                DB::exec(
                    'insert into designer_applications (user_id,display_name,desired_slug,bio,portfolio_url,social_links,design_types,uses_ai,agreement,status) values (?,?,?,?,?,?,?,?,?,"pending")',
                    [
                        H::user()['id'],
                        $values['display_name'],
                        $values['desired_slug'],
                ...
                DB::exec(
                    ...
                    [
                        null,
                        'submitted_designer_application',
                        'designer_application',
                        $applicationId,
                        json_encode(['user_id' => H::user()['id']]),
                    ]
                );
    ...
    public function home()
    {
        ...
        $stats = DB::row(
            'select count(*) product_count, coalesce(sum(sales_count),0) sales_count from products where designer_id=?',
            [$d['id']]
        );
    ...
    public function storeSettings()
    {
        ...
        if ($_POST) {
            ...
            if (!$errors) {
                DB::exec(
                    'update designers set display_name=?,store_slug=?,bio=?,website_url=?,social_links=?,announcement=?,seo_title=?,seo_description=?,avatar_path=coalesce(?,avatar_path),banner_path=coalesce(?,banner_path),updated_at=now() where id=? and user_id=?',
                    [
                        $display,
                        $slug,
                        $bio,
    ...
    private function productValues(array $existing = []): array
    {
        return [
            ...
            'slug' => H::slug(trim($_POST['slug'] ?? $existing['slug'] ?? '')),
            ...
            'category_id' => ($_POST['category_id'] ?? ($existing['category_id'] ?? '')) ?: null,
            'tags' => trim($_POST['tags'] ?? ''),
            'file_types' => $_POST['file_types'] ?? array_filter(
                array_map('trim', explode(',', $existing['file_types'] ?? ''))
            ),
            'commercial_license_enabled' => (int) isset($_POST['commercial_license_enabled']),
            'commercial_license_price' => $_POST['commercial_license_price'] ?? ($existing['commercial_license_price'] ?? '0.00'),
            'pod_allowed' => (int) isset($_POST['pod_allowed']),
            'ai_disclosure' => trim($_POST['ai_disclosure'] ?? $existing['ai_disclosure'] ?? ''),
            'seo_title' => trim($_POST['seo_title'] ?? $existing['seo_title'] ?? ''),
            'seo_description' => trim($_POST['seo_description'] ?? $existing['seo_description'] ?? ''),
        ];
    }
        ...
        foreach ($_FILES['preview_images']['name'] as $idx => $original) {
            ...
            if (move_uploaded_file($tmp, $dir . '/' . $name)) {
                ...
                DB::exec(
                    ...
                    [$productId, '/uploads/product_previews/' . $name, $alt, $sort]
                );
            }
        }
    }
    ...
    private function saveProductFiles(int $productId): bool
    {
        ...
        foreach ($_FILES['product_files']['name'] as $idx => $original) {
            ...
            if (move_uploaded_file($_FILES['product_files']['tmp_name'][$idx], $dir . '/' . $name)) {
                DB::exec(
                    'insert into product_files (product_id,storage_path,original_name,file_size,mime_type) values (?,?,?,?,?)',
                    [
                        $productId,
                        'products/' . $name,
                        $original,
                        $_FILES['product_files']['size'][$idx],
                        $_FILES['product_files']['type'][$idx] ?? 'application/octet-stream',
                    ]
                );
    ...
    {
        return (string) ($existing['title'] ?? '') !== (string) $values['title']
            || (string) ($existing['short_description'] ?? '') !== (string) $values['short_description']
            || (string) ($existing['description'] ?? '') !== (string) $values['description'];
    }
    ...
    public function products()
    {
        ...
        H::view('seller/products', [
            'status' => $status,
            'products' => DB::rows(
                'select p.*,c.name category_name,(select image_path from product_images pi where pi.product_id=p.id order by pi.sort_order,pi.id limit 1) thumbnail from products p left join categories c on c.id=p.category_id ' . $where . ' order by p.updated_at desc',
                $params
            ),
        ]);
    }
    ...
    public function editProduct($id = null)
    {
        ...
        if ($_POST) {
            ...
            if (!$errors) {
                ...
                if ($p) {
                    ...
                    DB::exec(
                        'insert into products (designer_id,category_id,title,slug,short_description,description,price,tags_text,file_types,commercial_license_enabled,commercial_license_price,pod_allowed,digital_resale_prohibited,ai_disclosure,seo_title,seo_description,status) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                        [
                            $d['id'],
                            $values['category_id'],
                            $values['title'],
        ...
        H::view('seller/edit_product', [
            ...
            'images' => $productId ? DB::rows(
                'select * from product_images where product_id=? order by sort_order,id',
                [$productId]
            ) : [],
            'files' => $productId ? DB::rows(
                'select * from product_files where product_id=? order by created_at desc',
                [$productId]
            ) : [],
    ...
    public function submitProduct($id)
    {
        ...
        DB::exec(
            'update products set status="pending_review",rejection_reason=null,updated_at=now() where id=? and designer_id=?',
            [$id, $this->d()['id']]
        );
    ...
    public function disableProduct($id)
    {
        ...
        DB::exec(
            'update products set status="disabled",updated_at=now() where id=? and designer_id=?',
            [$id, $this->d()['id']]
        );
    ...
    public function sales()
    {
        ...
        H::view('seller/sales', [
            'sales' => DB::rows(
                'select se.*,p.title,u.email from seller_earnings se join products p on p.id=se.product_id join users u on u.id=se.buyer_id where se.designer_id=?',
                [$this->d()['id']]
            ),
        ]);
    }
    ...
    public function referrals()
    {
        ...
        H::view('seller/referrals', [
            'refs' => DB::rows('select * from referrals where referrer_user_id=?', [H::user()['id']]),
        ]);
    }
