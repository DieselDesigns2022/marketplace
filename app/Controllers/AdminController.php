...
class AdminController
{
    ...
    private function retryAllocatedRefund(int $orderId,int $observationId):array
    {
        $affected=[];DB::begin();try{$ob=DB::row('select * from marketplace_refund_observations where id=? and order_id=? for update',[$observationId,$orderId]);if(!$ob||$ob['allocation_status']!=='allocated')throw new \DomainException('Allocated refund observation was not found.');$service=new MarketplaceRefundService();$affected=array_map('intval',array_column(DB::rows('select distinct oi.designer_id from marketplace_refund_allocations a join order_items oi on oi.id=a.order_item_id where a.refund_observation_id=?',[$observationId]),'designer_id'));(new StripeController)->reconcileRefundPayouts($orderId);$service->markReconciled($observationId);$service->clearReviewWhenFullyReconciled($orderId);DB::commit();}catch(Throwable $e){if(DB::pdo()->inTransaction())DB::rollBack();throw$e;}foreach($affected as $designer){(new CreatorRecognitionService)->recalculate($designer,false,true,'refund_allocation',null,'recognition:refund-allocation:observation:'.$observationId.':seller:'.$designer);StripeService::attemptPendingTransfersForDesigner($designer);}return$affected;
    }
    ...
    public function paymentLogs()
    {
        ...
        $summary = DB::row('select
            ...
            (select coalesce(round(sum(sp.transfer_amount_after_recovery),2),0) from seller_payouts sp join orders o on o.id=sp.order_id where sp.payout_status in ("transferred","recovery_applied") and o.stripe_checkout_session_id like "cs_live_%") seller_transfers_sent,
