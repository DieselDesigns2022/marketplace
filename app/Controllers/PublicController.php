...
class PublicController
{
    ...
    {
        $p = DB::row('select p.*,d.user_id seller_user_id,d.display_name,d.store_slug,d.creator_rank,d.founder_position,d.founder_active,d.average_rating,d.review_count,c.name category_name,c.slug category_slug from products p join designers d on d.id=p.designer_id left join categories c on c.id=p.category_id where p.slug=? and p.status="approved" and d.status="approved"', [$slug]) ?? H::abort(404);
        ...
        $productUrl = H::canonical('/product/'.$p['slug']);
        $schema = ['@context'=>'https://schema.org','@type'=>'Product','name'=>$p['title'],'description'=>$description,'url'=>$productUrl];
    ...
    private function designerSocialLinks(array $d): array
    {
        $fields = ['website_url'=>'Website','facebook_url'=>'Facebook','instagram_url'=>'Instagram','tiktok_url'=>'TikTok','pinterest_url'=>'Pinterest','etsy_url'=>'Etsy','shopify_url'=>'Shopify'];
        $links = [];
        foreach ($fields as $field => $label) {
            $url = trim((string)($d[$field] ?? ''));
            if ($url !== '' && filter_var($url, FILTER_VALIDATE_URL) && in_array(strtolower(parse_url($url, PHP_URL_SCHEME) ?: ''), ['http','https'], true)) $links[] = ['label'=>$label, 'url'=>$url];
        }
        return $links;
    }
