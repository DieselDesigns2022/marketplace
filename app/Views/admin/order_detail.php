...
<p>Status: <?=H::e($order['status'])?> · Payment: <strong><?=H::e($order['payment_status'] ?? $order['status'])?></strong> · Subtotal: <?=H::money($order['subtotal'] ?? 0)?> · Tax: <?=H::money($order['tax_amount'] ?? 0)?> · Total: <?=H::money($order['total'])?>
</p>
<p class="muted">Phase 10.3 seller sales tax is seller-reported/opt-in; marketplace facilitator and Stripe Tax automation are not enabled.</p>
...
<table>
    <tr>
        ...
        <th>Item Total</th><th>Seller Tax</th>
        <th>Commission</th>
    ...
    <?php foreach($items as $i):?>
        <tr>
           ...
           </td><td><?=H::money($i['tax_amount'] ?? 0)?><?php if(!empty($i['tax_snapshot'])):?><br><span class="muted"><?=H::e($i['tax_snapshot'])?></span><?php endif;?></td>
           <td>
           <?=H::money($i['commission_amount']??($i['total_price']*$i['commission_rate']))?>
           </td>
           <td>
           <?=H::money($i['seller_payout_amount'] ?? $i['seller_earning'] ?? ($i['total_price']-($i['total_price']*$i['commission_rate'])+($i['tax_amount'] ?? 0)))?><br><span class="muted">Commission basis excludes tax; seller payable includes collected seller tax.</span>
           </td>
