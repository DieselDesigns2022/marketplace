...
Phase 8.5 adds the marketplace licensing foundation with license types as included permissions/use-case options, not paid add-ons. Product price is the only buyer-facing price; Personal is always included; sellers can enable additional included permissions such as Commercial, POD, Wholesale, Fabric, VA, and Extended Commercial; buyers can select multiple included licenses; carts store normalized selected license keys; checkout validates each selected license server-side; and order items snapshot all selected included licenses while retaining `license_price` as a compatibility field set to `0.00`. A corrective migration zeroes existing product license prices after the original Phase 8.5 migration, and product listing cards now keep stable preview sizing when only one product is shown.
