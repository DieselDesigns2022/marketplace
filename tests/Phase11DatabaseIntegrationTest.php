...
$failures = [];
$check = function (bool $condition, string $message, array $actual = []) use (&$failures): void {
    echo ($condition ? 'PASS' : 'FAIL') . ": $message\n";
    if (!$condition) {
        if ($actual) {
            echo '  actual: ' . json_encode($actual, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR) . "\n";
        }
        $failures[] = $message;
    }
};
...
try {
    ...
    // Reproduce the originally deployed seller-referral shape. In particular, the
    // old named indexes are the only leftmost prefixes available to their FKs.
    $pdo->exec("create table seller_referral_payout_batches(id bigint primary key auto_increment,referrer_user_id bigint not null,period_start date not null,period_end date not null,amount_cents bigint not null,status enum('processing','paid','failed','not_ready') not null,stripe_transfer_id varchar(255) null,idempotency_key varchar(190) not null,failure_reason varchar(500) null,attempted_at timestamp null,succeeded_at timestamp null,created_at timestamp default current_timestamp,updated_at timestamp default current_timestamp on update current_timestamp,unique key seller_ref_payout_period_unique(referrer_user_id,period_start,period_end),unique key seller_ref_payout_key_unique(idempotency_key),key seller_ref_payout_processing_idx(status,attempted_at),constraint seller_ref_payout_user_fk foreign key(referrer_user_id) references users(id) on delete restrict) engine=InnoDB;
create table seller_referral_commission_ledger(id bigint primary key auto_increment,referral_id bigint not null,order_id bigint not null,order_item_id bigint not null,entry_type enum('accrual','refund_adjustment','recovery_adjustment') not null,amount_cents bigint not null,seller_earning_cents bigint not null default 0,related_entry_id bigint null,event_key varchar(190) not null,payout_item_id bigint null,created_at timestamp not null default current_timestamp,unique key seller_ref_commission_event_unique(event_key),key seller_ref_commission_unpaid_idx(referral_id,payout_item_id,created_at),key seller_ref_commission_order_idx(order_id,order_item_id),constraint seller_ref_commission_referral_fk foreign key(referral_id) references referrals(id) on delete restrict,constraint seller_ref_commission_order_fk foreign key(order_id) references orders(id) on delete restrict,constraint seller_ref_commission_item_fk foreign key(order_item_id) references order_items(id) on delete restrict,constraint seller_ref_commission_related_fk foreign key(related_entry_id) references seller_referral_commission_ledger(id) on delete restrict) engine=InnoDB;
create table seller_referral_payout_items(id bigint primary key auto_increment,batch_id bigint not null,ledger_entry_id bigint not null,amount_cents bigint not null,unique key seller_ref_payout_ledger_unique(ledger_entry_id),key seller_ref_payout_batch_idx(batch_id),constraint seller_ref_payout_item_batch_fk foreign key(batch_id) references seller_referral_payout_batches(id) on delete restrict,constraint seller_ref_payout_item_ledger_fk foreign key(ledger_entry_id) references seller_referral_commission_ledger(id) on delete restrict) engine=InnoDB;
...
create table seller_referral_transfer_attempts(id bigint primary key auto_increment,batch_id bigint not null,status enum('attempted','succeeded','failed') not null,stripe_transfer_id varchar(255) null,failure_reason varchar(500) null,attempted_at timestamp not null default current_timestamp,succeeded_at timestamp null,key seller_ref_attempt_batch_idx(batch_id),constraint seller_ref_attempt_batch_fk foreign key(batch_id) references seller_referral_payout_batches(id) on delete restrict) engine=InnoDB");
    $legacyFkIndexes = $pdo->query("select distinct index_name from information_schema.statistics where table_schema=database() and ((table_name='seller_referral_commission_ledger' and index_name='seller_ref_commission_unpaid_idx') or (table_name='seller_referral_payout_batches' and index_name='seller_ref_payout_period_unique') or (table_name='seller_referral_transfer_attempts' and index_name='seller_ref_attempt_batch_idx'))")->fetchAll(PDO::FETCH_COLUMN);
    $legacyFks = $pdo->query("select constraint_name from information_schema.key_column_usage where table_schema=database() and constraint_name in ('seller_ref_commission_referral_fk','seller_ref_commission_order_fk','seller_ref_commission_item_fk','seller_ref_commission_related_fk','seller_ref_commission_payout_item_fk','seller_ref_payout_user_fk','seller_ref_attempt_batch_fk') and referenced_table_name is not null")->fetchAll(PDO::FETCH_COLUMN);
    $check(count($legacyFkIndexes) === 3 && count($legacyFks) === 7, 'earlier seller-referral shape has the original FK-supporting indexes and foreign keys');
    $pdo->exec("insert into designers(id,user_id,status) values(900001,1,'approved');insert into orders(id,user_id,status,payment_status,subtotal,coupon_discount,tax_amount,credits_applied,total) values(100,2,'paid','paid',10,0,0,0,10);insert into order_items(id,order_id,designer_id,product_id,seller_payout_status) values(100,100,900001,1,'pending');insert into seller_referral_payout_batches(id,referrer_user_id,period_start,period_end,amount_cents,status,idempotency_key) values(1,1,'2026-07-01','2026-07-31',100,'processing','legacy-batch');insert into seller_referral_commission_ledger(id,referral_id,order_id,order_item_id,entry_type,amount_cents,seller_earning_cents,event_key) values(1,1,100,100,'accrual',100,900,'legacy-accrual');insert into seller_referral_payout_items(id,batch_id,ledger_entry_id,amount_cents) values(1,1,1,100);update seller_referral_commission_ledger set payout_item_id=1 where id=1;insert into seller_referral_transfer_attempts(id,batch_id,status) values(1,1,'attempted')");

    $pdo->exec($sellerReferralMigration);
    $pdo->exec('update seller_referral_commission_ledger set claimed_batch_id=1 where id=1');
    $requiredLedgerFks = $pdo->query("select constraint_name,column_name from information_schema.key_column_usage where table_schema=database() and table_name='seller_referral_commission_ledger' and constraint_name in ('seller_ref_commission_referral_fk','seller_ref_commission_order_fk','seller_ref_commission_item_fk','seller_ref_commission_related_fk','seller_ref_commission_claim_fk','seller_ref_commission_payout_item_fk') and referenced_table_name is not null order by constraint_name")->fetchAll(PDO::FETCH_KEY_PAIR);
    $requiredIndexes = $pdo->query("select table_name,index_name,group_concat(column_name order by seq_in_index) columns_list from information_schema.statistics where table_schema=database() and index_name in ('seller_ref_commission_unpaid_idx','seller_ref_commission_claim_idx','seller_ref_payout_period_sequence_unique','seller_ref_payout_processing_idx','seller_ref_attempt_batch_idx') group by table_name,index_name order by table_name,index_name")->fetchAll(PDO::FETCH_ASSOC);
    $ledgerFkPrefixes = $pdo->query("select distinct k.constraint_name,s.index_name from information_schema.key_column_usage k join information_schema.statistics s on s.table_schema=k.table_schema and s.table_name=k.table_name and s.column_name=k.column_name and s.seq_in_index=1 where k.table_schema=database() and k.table_name='seller_referral_commission_ledger' and k.referenced_table_name is not null order by k.constraint_name")->fetchAll(PDO::FETCH_KEY_PAIR);
    $check(count($requiredLedgerFks) === 6 && $requiredLedgerFks['seller_ref_commission_referral_fk'] === 'referral_id' && $requiredLedgerFks['seller_ref_commission_order_fk'] === 'order_id' && $requiredLedgerFks['seller_ref_commission_item_fk'] === 'order_item_id' && $requiredLedgerFks['seller_ref_commission_related_fk'] === 'related_entry_id' && $requiredLedgerFks['seller_ref_commission_claim_fk'] === 'claimed_batch_id' && $requiredLedgerFks['seller_ref_commission_payout_item_fk'] === 'payout_item_id', 'corrected migration retains every commission-ledger foreign key');
    $check(count($ledgerFkPrefixes) === 6, 'every commission-ledger foreign key retains a supporting leftmost index prefix');
    $check(array_column($requiredIndexes, 'columns_list', 'index_name') === ['seller_ref_commission_claim_idx'=>'claimed_batch_id','seller_ref_commission_unpaid_idx'=>'referral_id,payout_item_id,claimed_batch_id,created_at','seller_ref_payout_period_sequence_unique'=>'referrer_user_id,period_start,period_end,sequence_no','seller_ref_payout_processing_idx'=>'status,processing_started_at','seller_ref_attempt_batch_idx'=>'batch_id,attempted_at'], 'corrected migration installs every upgraded final index shape');
    $snapshot = static function (PDO $pdo): string {
        ...
        $data = ['referrals'=>$pdo->query('select * from referrals order by id')->fetchAll(PDO::FETCH_ASSOC),'credits'=>$pdo->query('select * from marketplace_credits order by id')->fetchAll(PDO::FETCH_ASSOC),'credit_ledger'=>$pdo->query('select * from credit_transactions order by id')->fetchAll(PDO::FETCH_ASSOC),'commission_ledger'=>$pdo->query('select * from seller_referral_commission_ledger order by id')->fetchAll(PDO::FETCH_ASSOC),'payout_batches'=>$pdo->query('select * from seller_referral_payout_batches order by id')->fetchAll(PDO::FETCH_ASSOC),'payout_items'=>$pdo->query('select * from seller_referral_payout_items order by id')->fetchAll(PDO::FETCH_ASSOC),'transfer_attempts'=>$pdo->query('select * from seller_referral_transfer_attempts order by id')->fetchAll(PDO::FETCH_ASSOC)];
    ...
    $pdo->exec('use `'.$fixture.'`');
    $schemaDifference=[];
    foreach (['columns','indexes','foreign_keys'] as $position=>$kind) {
        if ($migratedStructure[$position] !== $canonicalStructure[$position]) {
            $schemaDifference[$kind]=['migrated_only'=>array_values(array_udiff($migratedStructure[$position],$canonicalStructure[$position],static fn($a,$b)=>strcmp(json_encode($a),json_encode($b)))),'canonical_only'=>array_values(array_udiff($canonicalStructure[$position],$migratedStructure[$position],static fn($a,$b)=>strcmp(json_encode($a),json_encode($b))))];
        }
    }
    $check($migratedStructure===$canonicalStructure,'migrated Phase 11 column types, nullability, defaults, indexes, uniqueness, and foreign keys match canonical schema',$schemaDifference);
    ...
    $firstSellerQualification=$referrals->qualifySeller($sellerRewardOrder,$rewardDesigner,'reward:seller:'.$suffix);
    $replayedSellerQualification=$referrals->qualifySeller($sellerRewardOrder,$rewardDesigner,'reward:seller:'.$suffix);
    $sellerQualificationActual=['order_id'=>$sellerRewardOrder,'designer_id'=>$rewardDesigner,'item_ids'=>$pdo->query("select id from order_items where order_id=$sellerRewardOrder and designer_id=$rewardDesigner order by id")->fetchAll(PDO::FETCH_COLUMN),'first_result'=>$firstSellerQualification,'replay_result'=>$replayedSellerQualification,'referral'=>$pdo->query('select seller_reward_type,seller_qualifying_order_id,seller_qualifying_order_item_id,seller_reward_event_key from referrals where id='.$relationship)->fetch(PDO::FETCH_ASSOC)];
    $check($firstSellerQualification && !$replayedSellerQualification,'multiple same-seller items and replay reward seller once',$sellerQualificationActual);
    $rewardRow=$pdo->query('select * from referrals where id='.$relationship)->fetch(PDO::FETCH_ASSOC);
    $rewardSummary=array_intersect_key($rewardRow,array_flip(['seller_reward_type','seller_status','seller_referrer_reward_amount','seller_referred_reward_amount','seller_rewarded_at','buyer_referrer_reward_amount','buyer_referred_reward_amount']));
    $check($rewardRow['buyer_referrer_reward_amount']==='1.50' && $rewardRow['buyer_referred_reward_amount']==='1.50' && $rewardRow['seller_referrer_reward_amount']==='5.00' && $rewardRow['seller_referred_reward_amount']==='0.00','non-seller referrer receives $5 and referred seller receives no matching seller credit',$rewardSummary);
    ...
    $grantLedger=$pdo->query("select user_id,amount,idempotency_key,referral_id from credit_transactions where referral_id=$relationship and type='grant' order by amount,user_id,id")->fetchAll(PDO::FETCH_ASSOC);
    $check($grantRows===['1.50','1.50','5.00'],'ledger contains the buyer pair and only the seller referrer $5 grant exactly once',['rows'=>$grantLedger]);
    $pdo->rollBack();
    ...
    $firstAccrual=$commissionService->accrueOrder($commissionOrder,$newDesigner);
    $replayedAccrual=$commissionService->accrueOrder($commissionOrder,$newDesigner);
    $commissionRows=$pdo->query("select referral_id,order_id,order_item_id,entry_type,amount_cents,seller_earning_cents,event_key from seller_referral_commission_ledger where order_id=$commissionOrder order by order_item_id,id")->fetchAll(PDO::FETCH_ASSOC);
    $commissionAmounts=array_map(static fn(array $row):array=>[(int)$row['seller_earning_cents'],(int)$row['amount_cents']],$commissionRows);
    $check($firstAccrual===4 && $replayedAccrual===0 && $commissionAmounts===[[50,1],[149,1],[150,2]],'multiple items accrue exact rounded cents once under replay',['first_cents'=>$firstAccrual,'replay_cents'=>$replayedAccrual,'rows'=>$commissionRows]);
    ...
    $firstStop=$commissionService->permanentlyStop($newSeller,'store_disabled');
    $firstStopRow=$pdo->query('select commission_ended_at,commission_end_reason from referrals where id='.$sellerRelationship)->fetch(PDO::FETCH_ASSOC);
    $secondStop=$commissionService->permanentlyStop($newSeller,'store_deleted');
    $secondStopRow=$pdo->query('select commission_ended_at,commission_end_reason from referrals where id='.$sellerRelationship)->fetch(PDO::FETCH_ASSOC);
    $check($firstStop && !$secondStop && $firstStopRow===$secondStopRow,'first permanent stop wins and cannot be overwritten',['first_result'=>$firstStop,'second_result'=>$secondStop,'first_row'=>$firstStopRow,'second_row'=>$secondStopRow]);
