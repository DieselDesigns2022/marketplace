...
`CreatorRecognitionService` is the only recognition mutation path. It reads authoritative paid orders and the existing cumulative refund allocator, updates calculated state even behind an override, plans historical Founder positions by tenth timestamp/order/designer, and uses designer-then-mutex locking. Payment hooks and both meaningful-refund/replay-recovery hooks invoke it post-commit; the CLI supplies write-free dry-run review, silent apply backfill, and communicating daily UTC processing.
