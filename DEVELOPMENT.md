...
- Official Creative Moth branding watermark sources are deployment assets and are versioned at `storage/app/private/branding/watermark.png` and `storage/app/private/branding/extra-protection-watermark.png`. Generated seller preview files and retained seller originals must never be committed.
...
- Standard Creative Moth watermark rendering preserves transparent PNG alpha, centers the watermark, and renders it at 40% opacity. Listings may optionally add the full-preview `extra-protection-watermark.png` layer beneath it at 15% opacity. Regeneration always starts from a clean private original when one exists; imported remote previews are securely refetched from their stored source URL.
...
- Marketplace money math belongs in `MarketplaceFeeService`: group post-discount merchandise by seller, use integer cents, apply `PLATFORM_COMMISSION_PERCENT=9` plus `PLATFORM_COMMISSION_FIXED_CENTS=30` once per seller/order, and snapshot the result. Tax and store credit never reduce its merchandise basis.
...
Use test-mode Stripe keys only. `PLATFORM_COMMISSION_PERCENT` defaults to `9` and `PLATFORM_COMMISSION_FIXED_CENTS` defaults to `30`; do not commit real Stripe keys. Do not configure a Stripe application fee or Stripe Platform Pricing fee: Creative Moth computes its fee internally and absorbs payment processing. Historical order snapshots, including legacy percentage-only orders, are never repriced.
...
`STRIPE_WEBHOOK_SECRET` is required for the platform webhook. If a separate Stripe Connect webhook destination is used for `account.updated` and Stripe assigns it a different signing secret, set `STRIPE_CONNECT_WEBHOOK_SECRET` too. If connected-account events use the same webhook destination and secret, the optional Connect secret may remain empty. Current payouts use stored Phase 12.7 seller entitlement snapshots; Creative Moth absorbs Stripe processing and does not deduct it as a separate seller fee.

#### Phase 10 correction: transfer source transactions
Seller Stripe transfers include `source_transaction` when Creative Moth has the original `orders.stripe_charge_id`, plus `transfer_group=order_{orderId}` for order-level traceability. If a webhook-confirmed paid order does not have `stripe_charge_id` yet, the payout remains `pending_transfer` instead of being marked `transfer_failed` for charge/balance timing. Later `payment_intent.succeeded` or `charge.updated` webhook data can record the charge id and safely retry the exact stored Phase 12.7 transfer plan. Creative Moth absorbs Stripe processing; it is not reconciled as a seller deduction.
...
### Phase 12 development notes
`CreatorRecognitionService` is the only recognition mutation path. For current Phase 12.7 orders it reads authoritative durable item/seller refund allocations; the proportional cumulative refund allocator is retained only for legacy/historical compatibility. It updates calculated state even behind an override, plans historical Founder positions by tenth timestamp/order/designer, and uses designer-then-mutex locking. Payment hooks and both meaningful-refund/replay-recovery hooks invoke it post-commit; the CLI supplies write-free dry-run review, silent apply backfill, and communicating daily UTC processing.
