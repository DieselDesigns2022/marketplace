...
CREATE TABLE message_reports (id BIGINT PRIMARY KEY AUTO_INCREMENT,conversation_id BIGINT NOT NULL,reporter_user_id BIGINT NOT NULL,reason ENUM('abuse','spam','inappropriate','other') NOT NULL,details VARCHAR(1000) NULL,status ENUM('open','reviewing','resolved','dismissed') NOT NULL DEFAULT 'open',moderator_user_id BIGINT NULL,moderator_notes VARCHAR(1000) NULL,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,reviewed_at TIMESTAMP NULL,UNIQUE KEY message_report_reporter_unique(conversation_id,reporter_user_id),KEY message_reports_status_idx(status,created_at),CONSTRAINT message_report_conversation_fk FOREIGN KEY(conversation_id) REFERENCES message_conversations(id) ON DELETE CASCADE,CONSTRAINT message_report_reporter_fk FOREIGN KEY(reporter_user_id) REFERENCES users(id) ON DELETE RESTRICT,CONSTRAINT message_report_moderator_fk FOREIGN KEY(moderator_user_id) REFERENCES users(id) ON DELETE SET NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
